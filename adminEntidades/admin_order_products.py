class Admin_Order_Products:

    def __init__(self, db):
        self.__cnx = db

    def create(self, order_id, product_id, reordered):
        cursor = self.__cnx.cursor()
        insert = f"insert into orders(order_id, product_id, reordered) values ('{order_id}', ('{user_id}', ('{order_number}', ('{order_dow}', ('{days_since_prior_order}')"
        cursor.execute(insert)
        self.__cnx.commit()
        cursor.close()

    def update(self, order_id, product_id, reordered):
        cursor = self.__cnx.cursor()
    ...
    def get_all(self):
        cursor = self.__cnx.cursor()
        select = f"Select order_number from orders"
        cursor.execute(select)
        res = cursor.fetchall()
        cursor.close()
        for i in res:
